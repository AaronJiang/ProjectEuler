...
message. The balance for this method is using a sufficiently
...
for char in key:
    ...
    for cell in cells:
        if count % 3 == 1:
            result += chr(XOR(int(cell), ord(char[0])))
        elif count % 3 == 2:
            result += chr(XOR(int(cell), ord(char[1])))   
        elif count % 3 == 0:
            result += chr(XOR(int(cell), ord(char[2])))
        count += 1
    if result.count('the ') > 0 and result.count('and ') > 0:
